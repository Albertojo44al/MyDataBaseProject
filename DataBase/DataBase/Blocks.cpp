...
void dataBlocks::cleanBlock(int position) {
	FILE* file;
	...
	fwrite(block.blocks, md.blockSize, 1, file);
	fwrite(&block.next, sizeof(int), 1, file);
	fwrite(&block.actualPosition, sizeof(int), 1, file);
	fclose(file);
}